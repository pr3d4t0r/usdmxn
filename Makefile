...
VERSION=$(shell echo "from usdmxn import __VERSION__; print(__VERSION__)" | python)
...
# TODO: Use rm -Rfv $$(find $(PACKAGE) | awk '/__pycache__$$/') after the usdmxn
#       package is claimed to this project by PyPI.
clean:
	rm -Rf $(BUILD)/*
	rm -Rf $(DIST)/*
	rm -Rf $(MANPAGES)/*
	rm -Rfv $$(find usdmxn/ | awk '/__pycache__$$/')
...
manpage:
	...
	pandoc --standalone --to man README.md -o $(MANPAGES)/usdmxn.1
